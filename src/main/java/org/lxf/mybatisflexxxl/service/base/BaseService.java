package org.lxf.mybatisflexxxl.service.base;
...
import org.lxf.mybatisflexxxl.common.response.DeleteResult;
...
public interface BaseService<Entity> extends IService<Entity> {
     ...
     * @return 统一删除响应
     */
    default DeleteResult baseDelete(DeleteForm deleteForm, Class<Entity> clazz) {
        QueryWrapper queryWrapper = new QueryWrapper();
     ...
     */
    default <T extends ExcelExporter<T, E>, E> void baseExcelExport(HttpServletResponse response, ExcelExportForm excelExportForm, String fileName, Class<T> dtoClazz, Class<E> demoClazz) throws IOException {
        ...
        List<T> exportList = this.getMapper().paginateWithRelationsAs(excelExportForm.getCurrentPage(), excelExportForm.getPageSize(), queryWrapper, dtoClazz).getRecords();
        List<E> writeList = exportList.stream().map(data -> data.buildExcel(data)).toList();
    ...
    private void handleFilterCondition(QueryWrapper queryWrapper, String key, Object value) {
        ...
        if (FilterTypeEnum.ofType(conditions[1]).isEmpty()) {
            return;
        }
        switch (Objects.requireNonNull(FilterTypeEnum.ofType(conditions[1]).get())) {
    ...
    private void handleMetricCondition(String metric, QueryWrapper queryWrapper, Set<String> fieldSet) {
        ...
        if (MetricTypeEnum.ofType(conditions[1]).isEmpty()) {
            return;
        }
        switch (Objects.requireNonNull(MetricTypeEnum.ofType(conditions[1]).get())) {
